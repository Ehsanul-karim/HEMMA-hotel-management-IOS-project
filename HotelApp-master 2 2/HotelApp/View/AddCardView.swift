...
class AddCardView: UIView {
    ...
    @objc func AddCardAction() {
        let databaseRef = Database.database().reference()
                ...
                // Create a child reference using the user's UID
                guard let uid = Auth.auth().currentUser?.uid else {
                    return
                }

                let userBookingsRef = databaseRef.child("bookings").child(uid).childByAutoId()

                let booking = paymentViewController?.booking
                ...
                userBookingsRef.setValue(bookingDict) { (error, _) in
                    if let error = error {
                        print("Error writing booking to Firebase: \(error.localizedDescription)")
                    } else {
                        print("Booking successfully written to Firebase")
                    }
                }
