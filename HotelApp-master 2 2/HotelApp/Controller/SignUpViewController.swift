...
class SignUpViewController: UIViewController {
    ...
    var backgroundImage: UIImageView = {
        let image = UIImageView(image: UIImage(named: "hotelbg"))
    ...
    func signUpUser() {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authResult, error) in
            if let error = error {
                ...
                // Sign-up successful, save user information to Realtime Database
                let uid = authResult?.user.uid ?? ""
                let user = User(full_name: fullName, email: email, phone_number: phoneNumber, password: password)
                ApiService.sharedInstance.saveUser(uid: uid, user: user)
